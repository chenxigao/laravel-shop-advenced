      ...
      <div class="col-sm-7">
        ...
        @if($product->type === \App\Models\Product::TYPE_CROWDFUNDING)
          <div class="crowdfunding-info">
            ...
            <div class="total_amount"><span class="symbol">￥</span>{{ $product->crowdfunding->total_amount }}</div>
            ...
            @if($product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_FUNDING)
              <div>此项目必须在
                ...
                筹款将在
                <span class="text-red">{{ $product->crowdfunding->end_at->diffForHumans() }}</span> 结束
              </div>
              @endif
...
<script>
  $(document).ready(function () {
    ...
    //参与众筹按钮点击事件
    $('.btn_crowdfunding').click(function () {
        //判断是否选中 SKU
        if (!$('label.active input[name=skus]').$val()){
            swal('请先选择商品');
            return;
        }
        //把用户的地址以 JSON 的形式放入页面，赋值给 address 变量
        ...
        swal({
            ...
            var rep = {
                ...
                sku_id:$form.$('label.active input[name=skus]').val()
            };
            //调用众筹商品下单接口
            axios.post('{{ route('crowdfunding_orders.store') }}', $rep)
                .then(function (response) {
                    ...
                    if (error.response.status === 422){
                        ...
                        html += '<div>';
                        swal({contenct: $(html)[0], icon:'error'})
